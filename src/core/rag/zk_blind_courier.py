...
class BlindCourierEngine:
    ...
    def execute_blind_query(self, token: str, top_k: int = 5) -> Dict[str, Any]:
        ...
        action_plan, directive_str = self._generate_agent_action_plan(decrypted_intent, matched_tags, intent_hash)
        ...
        return {
            ...
            "agent_directive": directive_str,
            "action_plan": action_plan,
            "latency_ms": round(latency_ms, 2)
        }

    def _generate_agent_action_plan(self, intent: str, matched_tags: List[Dict[str, Any]], intent_hash: str) -> Tuple[List[Dict[str, Any]], str]:
        """Generates dynamic multi-stage execution plan for autonomous AI agents."""
        # Detect or default source file
        c_match = re.search(r'([\w\./\\-]+\.c(?:pp)?)', intent)
        if c_match:
            src_file = c_match.group(1)
        elif "telemetry" in intent.lower():
            src_file = "scratch/dummy_telemetry.c"
        else:
            src_file = "scratch/dummy_telemetry.c"

        src_stem = Path(src_file).stem
        mutated_file = f"scratch/{src_stem}_mutated.c" if src_stem != "dummy_telemetry" else "scratch/mutated_telemetry.c"
        bin_file = f"scratch/{src_stem}.exe" if src_stem != "dummy_telemetry" else "scratch/telemetry.exe"
        fbom_file = f"scratch/{src_stem}.fbom.json" if src_stem != "dummy_telemetry" else "scratch/telemetry.fbom.json"
        task_id = f"TASK-SWARM-TELEMETRY-01"
        agent_name = "Agent-Orchestrator"
        key_name = "telemetry_fbom"

        steps = [
            {
                "step": 1,
                "tool": "zk_polymorphic_mutator.py",
                "action": f"Pass {src_file} to zk_polymorphic_mutator.py to apply AST watermarking, string obfuscation, and CFF.",
                "command": f"python src/core/rag/zk_polymorphic_mutator.py -f {src_file} -o {mutated_file} --mutate"
            },
            {
                "step": 2,
                "tool": "zk_local_toolchain_builder.py",
                "action": f"Compile mutated code {mutated_file} with zk_local_toolchain_builder.py to build {bin_file} and generate FBOM.",
                "command": f"python src/core/rag/zk_local_toolchain_builder.py -f {mutated_file} -o {bin_file} --arch x64"
            },
            {
                "step": 3,
                "tool": "zk_swarm_blackboard.py",
                "action": f"Push generated FBOM manifest {fbom_file} to zk_swarm_blackboard.py state bus.",
                "command": f"python src/core/rag/zk_swarm_blackboard.py --push --task-id {task_id} --agent {agent_name} --key {key_name} --value-file {fbom_file}"
            }
        ]

        directive_str = (
            "Step 1: Pass this data to zk_polymorphic_mutator.py. "
            "Step 2: Compile the result with zk_local_toolchain_builder.py. "
            "Step 3: Push FBOM to zk_swarm_blackboard.py."
        )
        return steps, directive_str
...
def format_cli_output(result: Dict[str, Any]) -> str:
    ...
    if result.get("status") == "SUCCESS":
        directive = result.get("agent_directive", "Step 1: Pass this data to zk_polymorphic_mutator.py. Step 2: Compile the result with zk_local_toolchain_builder.py. Step 3: Push FBOM to zk_swarm_blackboard.py.")
        lines.extend([
            "=" * 80,
            " [TELEMETRY DATA FETCHED]",
            f" [AGENT DIRECTIVE]: {directive}",
            "-" * 80,
            " [DYNAMIC AGENT ACTION PLAN]:"
        ])
        for s in result.get("action_plan", []):
            lines.append(f"  * Step {s['step']}: {s['action']}")
            lines.append(f"    Command: {s['command']}")
        lines.append("=" * 80)
        lines.append(" [*] Verified Local Diagnostic Complete: Session Secured. Dynamic Agent Action Plan Dispatched.")
        lines.append("=" * 80)
    else:
        lines.append("=" * 80)
