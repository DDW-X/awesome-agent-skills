...
"""
        ...
        for line in lines:
            ...
            s_line = line.strip()
            if "{" in line and not s_line.startswith(("//", "/*", "*", "static const", "typedef", "struct", "union", "enum")) and "_zk_decode" not in line and "=" not in line:
                if not in_target_fn:
                    in_target_fn = True
                    dispatcher = """
    ...
    def inject_opaque_predicates(self, code: str) -> str:
        ...
        for line in lines:
            mutated_lines.append(line)
            s_line = line.strip()
            if "{" in line and not s_line.startswith(("//", "/*", "*", "static const", "typedef", "struct", "union", "enum")) and "=" not in line:
