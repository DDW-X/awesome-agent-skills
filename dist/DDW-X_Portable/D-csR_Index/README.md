...
- **Context Isolation**: When an AI agent performs a query, it interacts exclusively with `zk_hybrid_router.py`, which returns only abstract TAG identifiers.
...
```text
 ┌──────────────────────┐        Query String         ┌────────────────────────┐
 │ AI Agent Reasoning   │ ──────────────────────────► │ zk_hybrid_router.py    │
 │ (No Raw Text Ingest) │   Formulates Vector Query   │ (Calculates RRF Ranks)  │
 └──────────────────────┘                             └───────────┬─────────────┘
                                                                  │
                                                                  ▼
                                                      ┌─────────────────────────┐
                                                      │ Abstract TAGs Generated │
                                                      │  TAG-24AF-22, TAG-D5A0  │
                                                      └───────────┬─────────────┘
                                                                  │
                                                                  ▼
 ┌──────────────────────┐                             ┌─────────────────────────┐
 │ Secure Output File   │ ◄────────────────────────── │ zk_payload_compiler.py  │
 │ Workspace.md (Disk)  │   Direct SQLite Extraction  │ (Air-Gapped Local Fetch)│
 └──────────────────────┘                             └─────────────────────────┘
```

---

## 🚀 Quick Start: Querying the Database via CLI

```bash
# 1. Route semantic queries to abstract TAGs (Zero text leaked to stdout)
python src/core/rag/zk_hybrid_router.py "AST static vulnerability auditing" --top-k 5

# 2. Extract and compile payloads directly to disk
python src/core/rag/zk_payload_compiler.py TAG-0331-5F TAG-8E95-A0 --output Secure_Output_Workspace.md
```
